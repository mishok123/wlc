...
function setupMenuContext()
{
	...
	{
		// WeLiveCrypto integration: link from Forum -> Directory.
		// Default: if forum is at ".../forum/index.php" then directory is ".../directory/public/".
		$wlc_directory_url = !empty($modSettings['wlc_directory_url']) ? $modSettings['wlc_directory_url'] : '';
		if (empty($wlc_directory_url))
		{
			$forum_base = preg_replace('~/index\.php$~', '', $scripturl);
			$parent_base = preg_replace('~/forum/?$~', '', rtrim($forum_base, '/'));
			$wlc_directory_url = rtrim($parent_base, '/') . '/directory/public/';
		}

		$favicon_url = '';
		if (!empty($modSettings['favicon'])) {
			$favicon_url = '/wlc/directory/public/storage/' . $modSettings['favicon'];
		}
		...
		$buttons = array(
			...
			'directory' => array(
				'title' => 'Catalog',
				'href' => $wlc_directory_url,
				'show' => false,
				'icon' => !empty($favicon_url) ? '<img src="' . $favicon_url . '" alt="" style="width: 16px; height: 16px; vertical-align: middle;">' : 'home',
				'sub_buttons' => array(
				),
			),
			'search' => array(
			...
			'forumtheme' => array(
				'title' => 'Theme',
				'href' => $scripturl . '?action=forumtheme',
				'show' => $context['allow_admin'],
				'sub_buttons' => array(
				),
			),
		...
		foreach ($buttons as $act => $button)
			if (!empty($button['show']))
			{
				...
				if (isset($button['icon']) && strpos($button['icon'], '<') !== false)
					$button['icon'] = $button['icon'];
				elseif (isset($button['icon']) && file_exists($settings['theme_dir'] . '/images/' . $button['icon']))
